...
-- ---------------------------------------------------------------------------
-- 2. users  (UUID PK referencing Supabase auth.users — no password_hash)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 11. config  (key-value store for timing parameters etc.)
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    VALUES (
        ...
        COALESCE((NEW.raw_user_meta_data->>'role')::user_role, 'lab_user')
    );
    RETURN NEW;
END;
